    ...
    <div className='flex justify-center  items-center flex-col min-h-screen w-full'>
      ...
      <div className="container flex items-center justify-center flex-col my-7">
        <div className='flex justify-center items-center'>
          <div className='grid grid-cols-3 gap-8 max-mobile:grid-cols-2 max-mobile:mx-6'>
              ...
              <div key={product.id}>
                <div className={`${darkMode && "bg-primary text-white" || "bg-white"} rounded-md shadow-lg size-full`}>
                  ...
                  <Image classname="size-full" image={product.image_url} alt="Image Coffee" />
                    <h2 className={`text-center text-xl max-mobile:text-base text-primary font-bold ${darkMode && "text-white" || "text-primary"}`}>{product.name}</h2>
                    <p className='text-lg font-medium max-mobile:text-base'>{new Intl.NumberFormat("en-us", { style:"currency", currency:"USD", maximumFractionDigits: 0}).format(product.price)}</p>
                    </div>
                  </div>
              </div>
            )
